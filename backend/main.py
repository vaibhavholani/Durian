...
from models import (
    CategoryType, BrandType, GenerationRequest, FeedbackRequest,
    RegenerateRequest, UpdateContentRequest, JobStatus,
)
...
async def upload_and_generate(
    ...
    job_id = db.create_job(sku_ids=sku_ids, category=category.value, brand_type=brand_type.value)
    db.store_products(job_id, products)

    req = GenerationRequest(sku_ids=sku_ids, category=category, brand_type=brand_type)
...
async def _run_job(job_id: str, request: GenerationRequest):
    ...
    async def on_progress(sku_id, result):
        if result.error:
            db.store_error(job_id, sku_id, result.error)
        else:
            db.store_result(job_id, result)
...
async def regenerate(request: RegenerateRequest):
    ...
    current = job["results"].get(request.sku_id, {})
    ...
    if request.section:
        ...
        result = await generate_sku_content(product, request.category, gold)
        if result.error:
            raise HTTPException(500, result.error)
        db.update_result(request.job_id, request.sku_id, result.model_dump())
